...
(function () {
  ...
  /* ========== Lightbox Gallery (finger-tracking swipe) ==========
     ...
     Both #ewGalleryBtn, #ewGalleryTrigger and .bahar-view-all-btn open this
  ...
  const manifestoImgs = $$('.bahar-manifesto-img img');
  const featureImgs   = $$('.bahar-features-img img');
  ...
  $$('.bahar-view-all-btn').forEach(btn => {
  ...
  /* ========== FAQ accordion ========== */
  const faqList = $('#baharFaqList');
  if (faqList) {
    $$('.bahar-faq-q', faqList).forEach(btn => {
      btn.addEventListener('click', () => {
        const item = btn.parentElement;
        const answer = item.querySelector('.bahar-faq-a');
        const expanded = btn.getAttribute('aria-expanded') === 'true';
        // Close others
        $$('.bahar-faq-q', faqList).forEach(b => {
          if (b !== btn) {
            b.setAttribute('aria-expanded', 'false');
            const a = b.parentElement.querySelector('.bahar-faq-a');
