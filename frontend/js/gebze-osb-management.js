...
(function () {
  ...
  /* ========== Lightbox Gallery (finger-tracking swipe) ==========
     ...
     Both #ewGalleryBtn, #ewGalleryTrigger and .gebze-view-all-btn open this
  ...
  const manifestoImgs = $$('.gebze-manifesto-img img');
  const featureImgs   = $$('.gebze-features-img img');
  ...
  $$('.gebze-view-all-btn').forEach(btn => {
  ...
  /* ========== FAQ accordion ========== */
  const faqList = $('#gebzeFaqList');
  if (faqList) {
    $$('.gebze-faq-q', faqList).forEach(btn => {
      btn.addEventListener('click', () => {
        const item = btn.parentElement;
        const answer = item.querySelector('.gebze-faq-a');
        const expanded = btn.getAttribute('aria-expanded') === 'true';
        // Close others
        $$('.gebze-faq-q', faqList).forEach(b => {
          if (b !== btn) {
            b.setAttribute('aria-expanded', 'false');
            const a = b.parentElement.querySelector('.gebze-faq-a');
