...
(function () {
  ...
  /* ========== Lightbox Gallery (finger-tracking swipe) ==========
     ...
     Both #ewGalleryBtn, #ewGalleryTrigger and .mercan-view-all-btn open this
  ...
  const manifestoImgs = $$('.mercan-manifesto-img img');
  const featureImgs   = $$('.mercan-features-img img');
  ...
  $$('.mercan-view-all-btn').forEach(btn => {
  ...
  /* ========== FAQ accordion ========== */
  const faqList = $('#mercanFaqList');
  if (faqList) {
    $$('.mercan-faq-q', faqList).forEach(btn => {
      btn.addEventListener('click', () => {
        const item = btn.parentElement;
        const answer = item.querySelector('.mercan-faq-a');
        const expanded = btn.getAttribute('aria-expanded') === 'true';
        // Close others
        $$('.mercan-faq-q', faqList).forEach(b => {
          if (b !== btn) {
            b.setAttribute('aria-expanded', 'false');
            const a = b.parentElement.querySelector('.mercan-faq-a');
  ...
  if (!reduce) {
    $$('.mercan-slideshow').forEach(box => {
      ...
      const interval = parseInt(box.dataset.interval, 10) || 1500;
      let idx = 0;
      let timer = null;
      const tick = () => {
