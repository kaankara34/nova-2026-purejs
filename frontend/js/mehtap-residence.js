...
(function () {
  ...
  /* ========== Lightbox Gallery (finger-tracking swipe) ==========
     ...
     Both #ewGalleryBtn, #ewGalleryTrigger and .mehtap-view-all-btn open this
  ...
  const manifestoImgs = $$('.mehtap-manifesto-img img');
  const featureImgs   = $$('.mehtap-features-img img');
  ...
  $$('.mehtap-view-all-btn').forEach(btn => {
  ...
  /* ========== FAQ accordion ========== */
  const faqList = $('#mehtapFaqList');
  if (faqList) {
    $$('.mehtap-faq-q', faqList).forEach(btn => {
      btn.addEventListener('click', () => {
        const item = btn.parentElement;
        const answer = item.querySelector('.mehtap-faq-a');
        const expanded = btn.getAttribute('aria-expanded') === 'true';
        // Close others
        $$('.mehtap-faq-q', faqList).forEach(b => {
          if (b !== btn) {
            b.setAttribute('aria-expanded', 'false');
            const a = b.parentElement.querySelector('.mehtap-faq-a');
