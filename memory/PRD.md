...
### 2026-06 — Construction: engineering accuracy revision (seismic / ground / concrete)

Scope: technical content of the diagrams only. Section layouts, typography, colour system, spacing, scroll behaviour and animation concept unchanged. Helper: `scripts/cx_engineering.py`.

**Seismic Design schematic** — "Applied lateral action" → **Seismic action** with height-varying arrows + envelope and a "schematic — not a storey-force distribution" qualifier; "Drift — storey n" → **Interstorey drift — storey n**, drawn as a gauge proportional to the *relative* displacement between consecutive levels (storey 1 = lowest) against a common reference line; reference configuration labelled **Undeformed configuration / Undeformed axis** and the green curve **Deformed shape**; "Base reactions" → **Foundation reactions** with vertical, horizontal and overturning (moment) arrows; "Amplified for visualisation — not to scale" retained.

**Ground / Foundation figures** — 01 datum figure now distinguishes existing ground profile, finished ground level and **project datum ±0.00**; 02 basement figure uses **B01 / B0n — project specific** and **excavation / basement depth — project specific** with an explicit foundation level; 03 replaced with a system-neutral geotechnical excavation section (existing ground, retained ground, excavation, **excavation support system — project specific**, ground/groundwater actions, groundwater if encountered, adjacent structure/property, excavation-foundation level); 04 foundation photo replaced with `foundation-mat.webp` (reinforcement mat before/during casting) and overlaid with structural actions (vertical / horizontal / moments) → foundation system — project specific → supporting ground; 05 stratigraphy gained a borehole / investigation axis and a conditional groundwater symbol; 06 photo replaced with `ground-investigation.webp` (drilling rig) and overlaid with ground level, borehole / investigation and **investigation depth — project specific, governed by the zone of ground influence**. No fabricated soil classes, depths, levels or parameters anywhere.

**Concrete Performance** — added the thesis line "Concrete performance extends beyond compressive strength."; the material band now carries a restrained technical overlay (Specified material → Fresh concrete → Execution → Early-age development → Hardened concrete → Verified performance) whose active phase is derived from the same canonical rail index; added four minimal parameter groups (Input parameters / Execution parameters / Performance–verification / **Resulting performance** — strength · serviceability · durability); standards note replaced with the "Material → execution → performance" note citing **TS EN 206+A2 and TS 13515**. `.cx-flow-stage` grid column constrained to `minmax(0,1fr)` so the head/note no longer inherit the rail's width. No concrete classes, MPa, w/c, slump or cover values introduced.

Verified: all state sequences forward + reverse at 1920x1080, 1366x768, 390x844; ground readout 0 mismatches at all three; concrete stage fits at 1366x768 / 1280x800 / 1440x900 with nothing behind the header; SVG annotations 0 overflow (measured `textLength` condensing); zero horizontal overflow and zero console errors at 1024x768, 768x1024, 430x932 as well.

### 2026-06 — Construction: sections 05–12 approved copy replacement (content fixed, layout adaptive)

Scope (per user): replace the written content of **05 / Detailing … 12 / Result** with the final approved English copy, verbatim. Sections 01–04, header, nav, footer, all images / SVGs / 3D models and the animation concept untouched; layout, section sizing, pin durations and ScrollTrigger measurements adapt to the substantially longer copy. Helpers: `scripts/cx_content_0512.py` (rebuild, re-inserting the existing visual blocks unchanged), `scripts/cx_approved_0512.txt` + `scripts/cx_verify_0512.py` (integrity gate — **227 approved lines present, CONTENT INTEGRITY OK**).

**Content now on the page**
- 05 Reinforcement detailing: lede + 2 intro paragraphs, A.01–A.06 (2–3 paragraphs each, collapsible `.cx-prin-d` bodies tracked to the canonical active state), **Seismic detailing principle** note (TBDY 2018 / TS 500 / TS 708 / TS EN 13670).
- 06 Execution control: lede + 2 intro paragraphs, C.01–C.06 with full bullet lists, closing verification sentence.
- 07 Waterproofing continuity: lede + 2 intro paragraphs, nodes 01–05 (collapsible `.cx-wp-node-d`), **Water management principle**.
- 08 Envelope L.01–L.06 + regulatory closing; 09 Acoustics A.01–A.05 (`.cx-alist`, 4 existing waveform plots kept) + closing; 10 Coordination 6 discipline groups (`.cx-coordlist`) + **Coordination principle**; 11 Technical framework register rebuilt as 11 two-column rows (TBDY 2018 → Planlı Alanlar İmar Yönetmeliği) + closing; 12 Result title "Calculated. Detailed. Coordinated. Constructed." + 3 paragraphs + Nova Konut signature.

**Layout / motion adaptation**
- Pin lengths recalculated from the state count: cage 680vh, execution 660vh, waterproofing 560vh.
- `copyTracker(..., keepActive)`: the closing note is tracked into view but the active state is never scrolled above the reading window.
- **Bug fixed:** `.cx-insp-stage` had no explicit grid column, so head / drawing / label strip stretched to the 2981px rail width and the drawing was pushed off screen.
- **Bug fixed:** the edge-fade mask lived on the moving rail, hiding the last control column — moved to a static `.cx-insp-track` wrapper.
- `horizontalScrub(..., stepped)`: execution control rests on a measured stop per state and slides only in the last 30 % of the slice; the active index is derived from the rail translation (nearest stop) — position is the single source of truth. Label/diagram mapping C.01→Drawing, C.02→Formwork, C.03/C.04→Concrete, C.05→Enclosure, C.06→Completion with cumulative fill.
- Deterministic first paint for both rails (`apply(0)`); `(min-width:1201px) and (max-height:820px)` reads execution control as a non-pinned vertical list so no bullet is ever clipped; resize across a matchMedia breakpoint no longer jumps to the top (scroll anchor restored on `ScrollTrigger.refresh`); `[data-cx-in]` sweep reveals blocks skipped by a fast jump; `ScrollTrigger.refresh()` on `load` and `fonts.ready`.
...
## Construction page — responsive layout & overflow audit after the client's copy replacement (Jun 2026)

The client pushed a commit to `upstream/main` that replaced the approved technical copy of `frontend/construction.html` with a considerably longer version (Law No. 4708 / Yapı Denetimi Uygulama Yönetmeliği, TS 708, TS EN ISO 15630-1, TS EN 206 / TS 13515, TBDY 2018, TS 500, TS EN 13670 …). Pulled with `git merge --ff-only upstream/main` — only that file changed, nothing local was overwritten. The client's copy is now the source of truth (the older `cx_verify_0512.py` baseline no longer applies). Task: layout/responsive corrections only — no copy edits, no redesign, no new interaction system.

**Root causes found and fixed**
1. *Section 06 EXECUTION CONTROL heading cropped.* The pinned `.cx-insp-stage` is `height:100vh; overflow:hidden; align-content:center`; measured content need was 1393px at 1920x1080 and 1127px at 1440x900, so the surplus was cut off equally at the top and bottom. Fixed by (a) two-column head intro / verification lists / closing note above 1200px and (b) removing the pin for section 06 at **every** desktop height (the copy cannot fit any real viewport) — it now uses the existing `verticalSteps()` reading that 1366/1280 already used. Heading, eyebrow, subtitle, sequence SVG, stage row and all six C.01–C.06 cards are fully visible.
2. *Phones/tablets: approved copy scrolling out of a fixed reading window.* Sections 02/03/05/07 kept the desktop pinned reading below 1200px, where a single state (up to 506px) is taller than the 391–458px window — text was clipped and the top fade mask made it look like copy was disappearing. Below 1200px these four sections are now unpinned: drawing first (32vh), then every state in natural flow, `.cx-copywin` overflow visible, no tracking transform, no mask, all states at full opacity. Section heights became content-based, so the former 480–680vh dead scroll space is gone.
3. *Section 12 diagram overlapping the closing copy on phones.* The stage is now unpinned below 900px: diagram row (SVG ≤34vh, legible at 375px) then the copy row in flow. Scrim ramps rebalanced for legibility on both phone and desktop.
4. *Stale state after refresh mid-section (CRITICAL, iteration_28).* `verticalSteps()` seeded index 0 and only reacted to `onUpdate`, which never fires when a load already starts inside the section. It now has a `sync()` seeded from the live `ScrollTrigger` progress plus a `refresh` listener.
5. *Seismic drawing label pile-up.* The drawing still accumulates geometry, but only the active state keeps its `<text>` annotation.
6. *Closing `.cx-endnote` cut on the last state.* `copyTracker()` now runs the last state to the end of its column, unless the window cannot hold the active block and the note together (then the block keeps its position). Bottom breathing raised 24 → 40px, which also removed the 5–10px sub-line clipping at desktop transitions.
...
### design.html — visual language redefined (Jun 2026)
User feedback: the first image set read as monastic / rustic / pale minimalism. New brand DNA: contemporary urban residential luxury with layered refinement (Hermès Maison / Ralph Lauren Home register, without copying), informed by the uploaded Nova office photographs used as style reference only.

- All 16 assets in `media/images/design/` regenerated in the new language — same filenames and pixel dimensions (1264x848 / 848x1264), so layout, aspect ratios, spacing, typography and the animation system are untouched: urban stone-and-bronze Istanbul apartment façades, walnut-panelled lobby, oak-panelled living rooms with espresso leather and olive accents, bookmatched stone with bronze inlay, grain-matched walnut cabinetry with brushed brass, aged-bronze ironmongery, joinery lighting reveals with integrated vents, stone stair with bronze handrail, architectural urban terraces and courtyards, dusk façade.
- Copy, captions and alt text updated to the same material vocabulary (oak/walnut, bookmatched stone, aged bronze, brushed brass, leather, smoked glass, 2700–3000K integrated lighting, grain and vein matching, Bağdat Caddesi named once in context). Tone, section order and structure unchanged.
- Ground warmed #F2EEE7 → #F1EBE3 (body, html, header, theme-color); `.dz-lead` given a bottom margin.
- **Verification** — `testing_agent` iteration_32: 100% pass (6/6). No overflow at 11 viewports, 0 console errors, all 16 assets load, 61 copy reveals + 16 image reveals all reach opacity 1, all six index links scroll and take the active state, 0 clipped / 0 overlapping text blocks at 5 viewports, chrome and links intact. Small-type contrast already raised to rgba(45,43,40,.62–.70) (the report's contrast note was stale).

### design.html — copy replacement only (Jun 2026)
User supplied final copy; instruction: existing design.html is the fixed source of truth for structure and styling, replace visible text only. Applied via `scripts/apply_copy.py` (line-addressed replacement of 35 text nodes in `scripts/design_main.html`, each guarded by an opening-tag assertion) then `scripts/build_design.py`.

- No CSS, no JS, no images, no section order, no class/ID/hierarchy change. Element/tag sequence verified identical against the previous build except one added `<br />` inside the closing statement (line-break only, hidden on mobile by the existing rule).
- New headline set: "Architecture shaped by proportion." / "Architecture with purpose." / "Materials selected for performance and character." / "Layered interiors, precisely resolved." / closing statement now "Design quality depends on technical discipline." with the construction-and-engineering paragraph beneath it, keeping the existing "Construction & engineering" and "View projects" links.
- Interiors' four supplied paragraphs were composed into the existing three `<p>` slots and detail's two into the existing one, to avoid adding DOM nodes; lengths kept close so wrapping and composition are unchanged.
- **Self-verified** (no testing agent needed for a text-only change): 1600x950 and 390x844 — horizontal overflow 0, 0 hidden reveal elements after full scroll, clip scan across all headings/paragraphs/captions clean (only a 4px sub-pixel reading on .dz-h1, not visible), lint 0 errors.

### design.html — second copy pass, natural human voice (Jun 2026)
User supplied a rewritten, deliberately plainer copy set (no "rather than…", "is treated as…", "the objective is…", no slogan cadence, no luxury filler). Applied with `scripts/apply_copy2.py`: 40 line-addressed text replacements in `scripts/design_main.html`, each guarded by an opening-tag assertion, then `scripts/build_design.py`.

- Structure untouched: tag sequence identical to the previous build except two `<br />` line breaks added inside existing headings (detail statement, closing statement). No CSS/JS/image/spacing/animation change.
- New voice: "Architecture begins with proportion." / "Architecture with purpose." / "Materials chosen to last." / "Warm interiors, carefully resolved." / "The smallest details often say the most." / "Architecture continues outside." / "A design is only as good as the way it is built." / closing "Design and construction belong together."
- Interiors' four supplied paragraphs fitted into the existing three `<p>` slots and detail's two into the existing one (no DOM nodes added).
- **Self-verified**: 1600x950 and 390x844 — horizontal overflow 0, clip scan across all headings/paragraphs/captions clean, 0 hidden reveal elements after a full scroll, page height 16453 desktop / 15889 mobile (≈0.6% off the previous build, so section heights and wrapping stayed stable). Lint 0 errors.

### design.html — material register with 3D samples (Jun 2026)
User request: a small material register inside design.html — walnut, bookmatched stone, aged bronze, leather, smoked glass — each surface readable by origin and texture, with 3D and real texture (GSAP allowed).

- New section 06b `#material-register` added to `scripts/design_main.html` (between materiality and interiors) and rebuilt into `design.html`; no other section touched.
- `frontend/js/materials3d.js` (ES module, three + three/addons via a new importmap in the head): one RoundedBox sample plate, `MeshPhysicalMaterial` per surface (walnut/stone/leather use map + bumpMap, bronze is metallic, smoked glass uses transmission), RoomEnvironment PMREM (sigma 0.04) plus a warm 2700K key light, drag-to-turn with inertia and idle drift, GSAP half-turn transition on material change, IntersectionObserver scroll spy that keeps tabs and entries in sync, render loop gated on visibility, `prefers-reduced-motion` respected.
- 4 new material textures: `media/images/design/tex-walnut|stone|bronze|leather.webp` (1024²).
- Each entry reads Origin / Texture / Surface / Used for / How it ages in a hairline `dl` grid; entries dim to .55 when inactive on desktop.
- Two platform quirks handled: (1) `html, body { overflow-x: hidden }` in styles.css makes `position: sticky` impossible, so the sample column follows the reading column via a clamped JS `translateY` on `.dz-reg-stage`; (2) the page-level `overflow-x: clip` that used to mask the mobile full-bleed figures was removed (it also broke sticky/transform containment) and those figures now use `width: 100vw; margin-left: calc(50% - 50vw)` — horizontal overflow measured 0 at every viewport.
- **Verification** — `testing_agent` iteration_33 found one high-priority defect (follow() moved `.dz-reg-canvas` instead of `.dz-reg-stage`, so the canvas covered the tab row); fixed and re-tested in iteration_34: 100% pass — 40/40 real tab clicks at four scroll offsets, stage contained in the grid 8/8, 0 three.js console warnings, sample renders on desktop and mobile, drag rotates, spy runs forward and reverse, mobile keeps the sample in flow, all page regressions clean. Mobile tab row now carries a right-edge fade so the hidden fifth tab reads as scrollable.

### design.html — mobile walnut caption fit (Jun 2026)
User report: "mobilde burası sığmamış walnut" pointing at `<figcaption class="dz-cap">Walnut cabinetry — grain matched, brushed brass pull</figcaption>`.

- Root cause: below 1100px `.dz-wrap` gets `padding-right: calc(var(--dz-pad) + 20px)` so the reading column clears the vertical section rail. That makes `50%` no longer the viewport centre, so `margin-left: calc(50% - 50vw)` on the full-bleed children resolved 10px off-canvas (figure left = -10px) and the caption text ran under `#dzIndex`.
- Fix (css/design.css, `@media (max-width: 720px)`): the offset is now `calc(0px - var(--dz-pad))` and applied **only to direct children of `.dz-wrap`** (the three full-bleed figures that live outside a wrap keep the original `calc(50% - 50vw)`); `.dz-mat-b .dz-cap` and `.dz-land-a .dz-cap` get `padding: 0 calc(var(--dz-pad) + 20px) 0 var(--dz-pad)`; `.dz-reg-badge` keeps the page gutter (`right: var(--dz-pad)`).
- Verified at 375/390/430/720: caption text starts at 22px, wraps to two lines, right text edge 293/293/362 vs rail at 337/352/392; walnut figure full-bleed 0 → innerWidth; horizontal overflow 0; no figure outside the viewport.

### build-beyond-living.html — brand philosophy page (Jun 2026)
New page: NOVA / BUILD BEYOND LIVING manifesto, 10 narrative moments, user-supplied copy used verbatim.

- `scripts/bbl_main.html` holds the `<main>`; `scripts/build_bbl.py` assembles it with the shared NOVA chrome taken verbatim from `about.html` (utility bar, header, side menu, footer) → `frontend/build-beyond-living.html` (478 lines, `body.page-bbl`).
- `frontend/css/bbl.css` — all rules scoped to `.page-bbl`; ivory #F1EBE3 ground, dark #23211E material and closing plates, Cormorant Garamond + Montserrat; tablet MQ 1100px and a recomposed mobile MQ 767px (22px gutter, full-bleed architecture/detail figures, staggered material column).
- `frontend/js/bbl.js` — `bbl-lit` body class (header goes from transparent over the dark hero to an ivory bar), smooth scroll cue, GSAP/ScrollTrigger reveals for `[data-bb]` (opacity + 24px rise, per-section stagger), `[data-bb-late]` (a beat later) and `[data-bb-fig] img` (1.03 → 1 settle). Bails out before adding `html.bb-js` when GSAP is missing or `prefers-reduced-motion` is set, so copy is never hidden.
- `js/script.js`: `.bb-hero` added to the shared `heroEl` selector so the existing header transparency logic covers the new page.
- 13 original images generated with Gemini 3.1 Flash Image Preview in `media/images/bbl/` — hero, idea, architecture, home, mat-stone/timber/metal, detail, structure, finished, landscape, time, closing. No repeats.
- `scripts/link_bbl.py`: the existing `menu-item bb-living` entry now points at the page on all 19 pages (`data-testid="menu-link-bbl"`), and a `BUILD BEYOND LIVING` footer link was added next to DESIGN on the 7 pages that carry that row (`data-testid="footer-link-bbl"`).
- **Verification** — `testing_agent` iteration_35: 100% frontend pass. Zero horizontal overflow at 1440/1024/768/430/390/375, all 67 reveal targets end at opacity 1 after a full scroll, all 13 `<main>` images unique and loaded, all 10 sections render the exact copy, scroll cue and the three CTAs (construction / projects / design) work, hamburger + side menu + footer link work, header contrast correct in both states, reduced-motion load shows everything immediately, zero console errors. Nav addition regression-checked on 6 pages plus the 7 footer rows.
- Known, pre-existing: the footer `DESIGN / BUILD BEYOND LIVING / …` link row is hidden by the existing mobile footer CSS on phones (shared with DESIGN, not introduced here) — the page is reachable from the side menu on mobile.

### build-beyond-living.html — full rebuild: living with Nova (Jun 2026)
The user discarded the first Build Beyond Living page (architecture / material / detail narrative — too close to design.html) and supplied a complete new specification. The page now answers "what does it mean to live with Nova?": lifestyle, service culture, hospitality, amenities, Nova Membership, the neighbourhood and the relationship after handover. Architecture is the setting, living is the subject.

**Structure (10 moments + brand frame), supplied copy only**
01 Hero (100svh, dusk interior looking to the city) · 02 A Way of Living · 03 Everyday, Considered · 04 Nova Membership intro (dark charcoal + iPhone Home screen) · 05 Nova Membership scroll experience (sticky phone, 6 features) · 06 Beyond the Apartment (editorial amenities, no card grid) · 07 Residential Hospitality (warm stone) · 08 The Neighbourhood (full-bleed avenue + editorial sequence) · 09 Beyond Handover (typographic) · 10 Final Manifesto (dark, 2 CTAs) · minimal Nova / BUILD BEYOND LIVING frame before the existing footer.

**Files**
- `scripts/bbl_main.html` — rewritten `<main>`; `scripts/build_bbl.py` assembles it with the verbatim about.html chrome → `frontend/build-beyond-living.html` (679 lines).
- `frontend/css/bbl.css` — rewritten, all scoped to `.page-bbl`. Load-bearing rule at the top: `:root:has(body.page-bbl) { overflow-x: clip }` — the global `overflow-x: hidden` in styles.css would otherwise make `position: sticky` impossible.
- `frontend/js/bbl.js` — rewritten: `bbl-lit` header ground, in-page smooth anchors, `sync()` that maps the reader position onto the active feature/app screen, GSAP reveals (opacity + 26px rise, clip-path image settle), all skipped under `prefers-reduced-motion`.
- `frontend/media/images/bbl/` — 14 new original lifestyle images (Gemini 3.1 Flash Image Preview) + a 760w `-sm` variant each, wired through `srcset`/`sizes`: hero, arrival, concierge, lounge, amenity-pool/-fitness/-cinema/-courtyard, hospitality, nb-avenue/-dining/-cafe/-coast, closing. Residential Istanbul only — no tourist landmarks, no luxury clichés.
- `scripts/fetch_bbl_images.py` — downloads/derives the image set.
- `scripts/bbl_verify_copy.py` — copy gate: **139/139 approved lines present verbatim**.

**Nova Membership**
Hand-built app UI in HTML/CSS (no screenshots): ivory/charcoal with Nova green used sparingly, Home / Services / Amenities / Support nav, and six screens — Concierge, Technical Support, Amenities, Home Services, Reservations, Support. Reservations is explicitly a concierge request ("Concierge Confirming"), never a guaranteed booking. Desktop keeps the phone sticky beside the scrolling features; below 768px the stage sticks on its own dark band and the copy passes under it.

**Verification** — `testing_agent` iteration_36 at 320/375/390/430/768/1024/1440: zero horizontal overflow, zero console errors, all 14 images load, no reveal stuck at opacity 0, sticky stage resolves `sticky` at every width and cycles features/screens 0→5 in order with one screen on at a time, no scroll trapping, all app copy present, CTAs (`bbl-scroll-cue`, `bbl-cta-projects` → projects.html, `bbl-cta-membership` → in-page `#nova-membership`) work, menu/footer BBL links work, header contrast correct in both grounds, reduced-motion correct, and index/design/construction regression clean. One HIGH defect was reported and fixed: the Reservations screen (and Amenities at 320px) overflowed the small stage phone and clipped the bottom nav — the mobile stage app spacing was tightened and `.bb-ui-nav` pinned to the device; re-measured at 320/375/390/430, every one of the six screens now sits 5–6px inside the device screen.

### build-beyond-living.html — Nova branding + Membership app colour system (Jun 2026)
- **Header logo fixed**: the previous `filter: invert(...)` treatment turned the mark black-and-white. The page now uses the same asset and treatment as design.html — `media/images/nova-logo-dark.png`, no filter, header always the ivory bar. The `bbl-lit` toggle was removed from `bbl.js` and its CSS. Verified byte-identical src/filter/size against design.html, and a canvas sample confirms the green circular "O" renders (34k green pixels).
- **App branding**: every service screen carries a small `NOVA MEMBERSHIP` wordmark above a serif screen title; Concierge adds "How can we assist?". The home screen leads with `NOVA MEMBERSHIP` / "Good evening" / "Your residence, connected." then Quick Access and Your Requests. `build_bbl.py` also repairs the stray slash in the shared logo tag for this page only.
- **Colour system (user revision)**: after a first pass with a different accent per screen, the user simplified it — **every app screen now sits on `#F2EDE2` with the olive accent (`89 99 72`) and tobacco detail, and only Reservations keeps its oxblood accent (`107 53 52`)**. The device frame stays dark graphite at every screen; no gradients inside the screen.
- **Reservations**: fields read Strada Suadiye / Tonight / 20:30 / 2; the active card leads with **Strada Suadiye** (14.4px serif) over "Restaurant Reservation" then the status **CONCIERGE CONFIRMING** in oxblood. "Confirmed" / "Guaranteed" / "Booked" appear nowhere on the page.
- **Legibility**: the app is now scale-invariant (`font-size: calc(var(--bb-phone-w) * .0415)`, mobile stage `.056`), the mobile stage phone grew to `min(50vw, 202px)` (58vw below 360px) and the smallest labels were raised — in-app text went from a 4.9px worst case to 7.4–8.5px. Home Services and Support rows breathe wider so the sparse screens no longer leave a gap above the nav.
- **The user also asked to remove the Nova logo image from inside the app** (it was briefly added as `media/images/nova-app-mark.png`); the app now brands itself with the wordmark only. The asset stays in the repo, unused.
- **Verification**: `testing_agent` iteration_37 passed all 10 requested checks with zero console errors (logo, app branding, per-screen accents, crossfade sync, Reservations hierarchy, mobile containment, page regression, other pages untouched, reduced motion). Its three optional design notes were then addressed. After the user's colour simplification and logo removal, re-measured at 320/375/390/430/768/1024/1440: zero horizontal overflow, all six screens 7–13px inside the device box, uniform `#F2EDE2` ground with the oxblood exception intact, copy gate still 139/139.

### contact.html — corporate contact page (Jun 2026)
Built, then reworked at the user's request: the first pass was too editorial (large serif "A direct line to Nova.", a closing manifesto line, oversized type). The shipped version is compact, corporate and information-first — closer to a large developer/investment company than a design studio.

- `scripts/contact_main.html` + `scripts/build_contact.py` → `frontend/contact.html` (325 lines, `body.page-contact`), chrome (utility bar, header, side menu, footer) taken verbatim from about.html, logo swapped to `nova-logo-dark.png` like design.html.
- `frontend/css/contact.css` — sans-serif throughout, no serif drama: page title 26–38px uppercase, lead 14–15.5px, labels 10.5px, contact values 17–23px, office names 13–15px, addresses 13–14.5px. Hierarchy comes from thin 1px rules, a strict 3-column office grid (2-up on tablet, 1-up on mobile) and section padding of 48–96px.
- Content is only: CONTACT / "For enquiries and project information." · EMAIL `iletisim@nova.istanbul` (`mailto:`) · PHONE `+90 (533) 506 1972` (`tel:+905335061972`) · OFFICES with the three supplied addresses verbatim (Bağdat Caddesi — 1, Bağdat Caddesi — 2, Etiler) · one wide secondary street image (reuses `bbl/nb-avenue.webp`, 18:6 capped at 300px). No form, no map embed, no closing quote, no duplicated contact block, no invented data.
